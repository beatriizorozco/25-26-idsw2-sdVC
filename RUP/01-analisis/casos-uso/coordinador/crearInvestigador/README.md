...
2. **Solicitud principal**: `CrearInvestigadorView` -> `InvestigadorController.crearInvestigador(datos)`.
3. **Validación previa**: `CrearInvestigadorView` -> `InvestigadorController.validarInvestigador(datos)`.
4. **Consulta de consistencia**: `InvestigadorController` -> `InvestigadorRepository.existeDuplicado(datos)`.
5. **Persistencia**: `InvestigadorController` -> `InvestigadorRepository.guardar(entidad)`.
...
|Coordinar reglas del caso de uso|`InvestigadorController`|`crearInvestigador(datos)`|
|Aplicar permisos y validaciones|`InvestigadorController`|`validarInvestigador(datos)`|
