...
Analizar la colaboración necesaria para presentar a Coordinador el detalle de convocatoria y sus acciones disponibles. El análisis identifica clases Boundary, Control y Entity, sus responsabilidades y colaboraciones necesarias para cumplir con el caso de uso `abrirConvocatoria()`.
...
**Responsabilidades**:
- Recibir la solicitud `abrirConvocatoria()` del Coordinador.
...
**Colaboraciones**:
- **Entrada**: Recibe `abrirConvocatoria()` desde el estado de contexto correspondiente.
...
**Estereotipo**: Control  
**Responsabilidades**:
- Coordinar la ejecución del caso de uso `abrirConvocatoria()`.
- Aplicar reglas de permisos del Coordinador.
- Validar datos o criterios antes de acceder a las entidades.
- Servir como intermediario entre la vista y el repositorio.

**Colaboraciones**:
- **Vista**: Responde a solicitudes de `DetalleConvocatoriaView`.
...
1. **Inicio**: Estado de contexto -> `DetalleConvocatoriaView.abrirConvocatoria()`.
2. **Solicitud principal**: `DetalleConvocatoriaView` -> `ConvocatoriaController.obtenerConvocatoria(id)`.
3. **Acceso a datos**: `DetalleConvocatoriaView` -> `ConvocatoriaController.prepararAccionesDisponibles(coordinador)`.
4. **Preparación de acciones**: `ConvocatoriaController` -> `ConvocatoriaRepository.obtenerPorId(id)`.
5. **Verificación de permisos**: `ConvocatoriaController` -> `ConvocatoriaRepository.verificarPermisos(actor)`.
6. **Finalización**: `DetalleConvocatoriaView` devuelve el control al estado de navegación definido.
...
|-|-|-|
|Atender la solicitud `abrirConvocatoria()`|`DetalleConvocatoriaView`|Recibe la acción del Coordinador|
|Coordinar reglas del caso de uso|`ConvocatoriaController`|`obtenerConvocatoria(id)`|
|Aplicar permisos y validaciones|`ConvocatoriaController`|`prepararAccionesDisponibles(coordinador)`|
|Acceder a datos de convocatorias|`ConvocatoriaRepository`|`obtenerPorId(id)`, `verificarPermisos(actor)`|
...
- **abrirConvocatorias()**: colaboración relacionada desde la navegación del caso de uso.
...
### MVC pattern
Separación clara entre presentación (`DetalleConvocatoriaView`), lógica de aplicación (`ConvocatoriaController`) y datos (`Convocatoria`, `ConvocatoriaRepository`).
...
- [Especificación detallada: abrirConvocatoria()](/RUP/00-casos-uso/02-detalle/coordinador/abrirConvocatoria/README.md)
