...
Analizar la colaboración necesaria para presentar a Investigador el detalle de publicación propia y sus acciones disponibles. El análisis identifica clases Boundary, Control y Entity, sus responsabilidades y colaboraciones necesarias para cumplir con el caso de uso `abrirMiPublicacion()`.

## Diagrama de colaboración

<div align=center>

|![Análisis: abrirMiPublicacion()](/images/RUP/01-analisis/casos-uso/investigador/abrirMiPublicacion/abrirMiPublicacion-analisis.svg)|
|-|
...
**Responsabilidades**:
- Recibir la solicitud `abrirMiPublicacion()` del Investigador.
...
**Colaboraciones**:
- **Entrada**: Recibe `abrirMiPublicacion()` desde el estado de contexto correspondiente.
...
**Estereotipo**: Control  
**Responsabilidades**:
- Coordinar la ejecución del caso de uso `abrirMiPublicacion()`.
...
1. **Inicio**: Estado de contexto -> `DetallePublicacionView.abrirMiPublicacion()`.
2. **Solicitud principal**: `DetallePublicacionView` -> `PublicacionController.obtenerPublicacion(id)`.
3. **Acceso a datos**: `DetallePublicacionView` -> `PublicacionController.prepararAccionesDisponibles(investigador)`.
...
|-|-|-|
|Atender la solicitud `abrirMiPublicacion()`|`DetallePublicacionView`|Recibe la acción del Investigador|
|Coordinar reglas del caso de uso|`PublicacionController`|`obtenerPublicacion(id)`|
|Aplicar permisos y validaciones|`PublicacionController`|`prepararAccionesDisponibles(investigador)`|
|Acceder a datos de publicaciones propias|`PublicacionRepository`|`obtenerPorId(id)`, `verificarPermisos(actor)`|
...
- [Especificación detallada: abrirMiPublicacion()](/RUP/00-casos-uso/02-detalle/investigador/abrirMiPublicacion/README.md)
